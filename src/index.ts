import { Query as MongooseQuery, Document, plugin, Schema } from "mongoose";
 ...
 * @public
 */
export type PagingCursor = Partial<Record<string, unknown>>;

/** Info about how to page forward and backward
 * @public
 */
export type PagingInfo<Cursor extends PagingCursor = PagingCursor> = {
	first?: number;
	last?: number;
	after?: Cursor | null;
	before?: Cursor | null;
...
const edgesToReturn = function EdgesToReturn<
	...
	Q extends DefaultRelayQuery = DefaultRelayQuery
>(query: Q, { before, after, first, last }: PagingInfo<Cursor>): Q {
	let edges = applyCursorsToEdges(query, before, after);
	...
	if (last) {
		...
		edges.limit(last);
		const currentSort: Record<string, number> = edges.getOptions().sort ?? {
			_id: 1,
		};
		// _id:
		edges.sort(
...
const applyCursorsToEdges = <
	MyCursor extends PagingCursor = PagingCursor,
	Q extends DefaultRelayQuery = DefaultRelayQuery
>(
	allEdges: Q,
	...
	const sortBy = edges.getOptions().sort;
	// If after is set:
	if (after) {
		...
		Object.entries(after).forEach(([key, value]) => {
			edges.find({
				[key]: !sortBy || sortBy[key] === 1 ? { $gt: value } : { $lt: value },
	...
	if (before) {
		...
		Object.entries(before).forEach(([key, value]) => {
			edges.find({
				[key]: !!sortBy && sortBy[key] === 1 ? { $lt: value } : { $gt: value },
			});
...
type ElementOfArray<Array extends any[]> = Array extends (infer T)[]
	? T
	: never;
...
export interface RelayResult<
	...
	Cursor extends PagingCursor
> {
	edges: { node: ElementOfArray<Nodes>; cursor: Cursor }[];
	nodes: Nodes;
	pageInfo: {
		count: number;
		hasNextPage: boolean;
...
type MongooseRelayConnectionToCursor<
	Q extends DefaultRelayQuery,
	Cursor extends PagingCursor
> = (document: MongooseRelayDocument<Q>) => Cursor;

/** A helper generic type which when given a {@link DefaultRelayQuery} and {@link PagingCursor} construct its corresponding toCursor type. */
type MongooseRelayPaginateInfo<
	Q extends DefaultRelayQuery,
	Cursor extends PagingCursor
> = {
	toCursor?: MongooseRelayConnectionToCursor<Q, Cursor>;
} & PagingInfo;
 ...
 * @returns
 */
export function relayPaginate<
	Cursor extends PagingCursor,
	Q extends DefaultRelayQuery
>(
	query: Q,
	{ toCursor, ...pagingInfo }: MongooseRelayPaginateInfo<Q, Cursor>
): MongooseQuery<
	RelayResult<MongooseRelayDocument<Q>[], Cursor>,
	QueryDocType<Q>,
	QueryHelpers<Q>,
	QueryRawDocType<Q>
> &
	QueryHelpers<Q> {
	query = edgesToReturn(query, pagingInfo);
	return query.transform(async (nodes) => {
		const countQuery = query.clone().estimatedDocumentCount();
		const count = await countQuery;

		const anyLimit = pagingInfo.last ?? pagingInfo.first ?? DEFAULT_LIMIT;
		return relayResultFromNodes(
			toCursor as unknown as any,
			{
				count,
				hasNextPage:
...
>(
	toCursor: MongooseRelayConnectionToCursor<DefaultRelayQuery, Cursor>,
	{
		count,
		hasNextPage,
		hasPreviousPage,
	}: Pick<
		RelayResult<Result[], Cursor>["pageInfo"],
		"count" | "hasNextPage" | "hasPreviousPage"
	>,
	nodes: Result[]
): RelayResult<Result[], Cursor> {
	return {
 ...
 */
export function alterNodeOnResult<Result, U, Cursor extends PagingCursor>(
	doc: RelayResult<U[], Cursor>,
...
declare module "mongoose" {
	interface Query<ResultType, DocType, THelpers = {}, RawDocType = DocType> {
		 ...
		 * @returns
		 */
		relayPaginate<Cursor extends PagingCursor, Q extends DefaultRelayQuery>(
			this: Q,
			{ toCursor, ...pagingInfo }: MongooseRelayPaginateInfo<Q, Cursor>
		): MongooseQuery<
			RelayResult<MongooseRelayDocument<Q>[], Cursor>,
...
function relayPaginatePlugin(schema: Schema) {
	(schema.query as any).relayPaginate = function (paging: any) {
